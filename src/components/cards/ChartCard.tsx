...
function Figure({ payload }: { payload: ChartPayload }) {
  ...
  switch (type) {
    ...
    case "heatmap": {
      // Custom SVG heatmap: rows = data items, columns = series keys
      const heatCols = series.map((s) => s.key);
      const allVals = data.flatMap((row) => heatCols.map((k) => Number(row[k]) || 0));
      const maxVal = Math.max(...allVals, 1);
      const cellH = 32;
      const labelW = 110;
      const cellW = Math.max(48, Math.floor((800 - labelW) / heatCols.length));
      const svgH = data.length * cellH + 40;
      const svgW = labelW + heatCols.length * cellW;
          ...
          <svg width={svgW} height={svgH} style={{ fontFamily: "inherit" }}>
            ...
            {data.map((row, ri) => (
              <g key={ri}>
                ...
                <text
                  x={labelW - 6}
                  y={40 + ri * cellH + cellH / 2}
                  textAnchor="end"
                  dominantBaseline="middle"
                  fontSize={10}
                  fill="var(--color-muted-foreground)"
                >
                  {String(row[xKey] ?? "").substring(0, 14)}
                </text>
                {heatCols.map((col, ci) => {
